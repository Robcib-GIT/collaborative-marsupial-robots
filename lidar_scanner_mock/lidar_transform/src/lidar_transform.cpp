...
void sendPointCloud()
{
  ...
  //pcl::PointCloud<pcl::PointXYZ> scan3dMsg;
  sensor_msgs::PointCloud scan3dMsg;
...
// Lectura del ángulo retornado true angle
// Lectura del ángulo retornado true angle
...
// se envía el águlo junto con el la nube para
// armar la matriz de puntos 3D
...
int main(int argc, char** argv)
{
  ...
  ros::Subscriber scanSubscriber = n.subscribe("/scan2", 1000, scanCallback);
  ...
  while (ros::ok())
  {
    //	 ROS_INFO("while_tansform >>>>>>>>>>");
    if (requestNextAngle)
    {
      requestNextAngle = false;
      //g_setAngle += (M_PI / 180);
      g_setAngle = g_setAngle + 0.005;
    //   ROS_INFO("angle %.3f",g_setAngle);

      if (g_setAngle > M_PI)
        g_setAngle = 0;
// Llamada a la función que publica el
// tópico setPoint del ángulo
